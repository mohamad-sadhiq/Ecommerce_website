...
<head>
    ...
    <style>
        :root {
            ...
            --text-gray: #666666; --brand-color: #7b1e2e; --border-light: #eaeaea;
            --error-color: #d9534f; --success-color: #4cae4c;
        }
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .auth-image-panel { flex: 1.2; position: relative; overflow: hidden; border-right: 1px solid var(--border-light); background: var(--bg-white); }
        .auth-image-panel img { width: 100%; height: 100%; object-fit: cover; object-position: center; animation: floatJewelry 6s ease-in-out infinite; transform: scale(1.05); }
        @keyframes floatJewelry { 0% { transform: scale(1.05) translateY(0px); } 50% { transform: scale(1.05) translateY(-15px); } 100% { transform: scale(1.05) translateY(0px); } }
        ...
        .input-group input:focus ~ label, .input-group input:valid ~ label { top: -5px; font-size: 11px; color: var(--brand-color); font-weight: 500; text-transform: uppercase; letter-spacing: 1px; }
        ...
        <div class="auth-form-panel">
            ...
            <div class="form-content-wrapper">
                ...
                <c:if test="${param.logout == 'true'}">
                    <div class="alert-box alert-success"><i class="fas fa-info-circle" style="margin-right: 5px;"></i> You have been successfully logged out.</div>
                </c:if>
                ...
                <p class="auth-footer">
                    ...
                    <c:choose>
                        <c:when test="${not empty redirect}">
                            <a href="/register?redirect=${redirect}">Register here</a>
                        </c:when>
                        <c:otherwise>
