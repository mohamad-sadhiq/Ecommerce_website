...
<style>
    ...
    /* Navigation Stack & Invisible Scrollbar */
    .nav-menu {
        flex: 1;
        display: flex;
        flex-direction: column;
        padding: 25px 0;
        overflow-y: auto;
        /* Hides scrollbar across different browsers while keeping scroll functional */
    ...
    .nav-label { font-size: 11px; text-transform: uppercase; letter-spacing: 2px; color: rgba(255,255,255,0.4); margin: 20px 20px 10px; font-weight: 600; }

    /* User-Friendly Floating Button Links */
    .nav-item {
        display: flex; align-items: center; gap: 15px;
        padding: 12px 20px;
        margin: 4px 15px; /* Creates the floating button effect by pulling away from edges */
        color: #e0d0e5; font-size: 14px; font-weight: 500; cursor: pointer; transition: all 0.3s ease;
        text-decoration: none;
        border-radius: 8px; /* Smooth rounded button look */
    }
    .nav-item i { font-size: 16px; width: 20px; text-align: center; }

    /* Maroon Button Active/Hover State */
    .nav-item:hover, .nav-item.active {
        background: #7b1e2e; /* Luxury Maroon Color */
        color: #ffffff;
        box-shadow: 0 4px 12px rgba(123, 30, 46, 0.4); /* Elegant glow */
        transform: translateX(4px); /* Smooth interaction nudge */
    }
    ...
    .logout-btn:hover {
        background: rgba(220, 53, 69, 0.15); /* Danger tint */
        color: #ff6b6b;
        box-shadow: none;
    }
</style>
